...
Fraction cuDeltaCal(Fraction paraList[4], Fraction *dhDiff, Fraction *h_sqr) {
	...
	//delta_sqr_nu2 = fReduce(fMul( fIntToFrac_numAdd(2), fMul(paraList[0], paraList[2])));
	//delta_sqr_nu = fReduce(fDiv(fPow(paraList[1],2 ), fReduce(fMul(fIntToFrac_numAdd(2), fMul(paraList[0], paraList[2])))));
	//delta_sqr_de = fReduce(fMul(fIntToFrac_numAdd(9), fPow(paraList[0], 2))); 
	//delta_sqr = fReduce(fDiv(delta_sqr_nu, delta_sqr_de));
	delta_sqr = fReduce(fDiv(fReduce(fDiv(fPow(paraList[1], 2), fReduce(fMul(fIntToFrac_numAdd(2), fMul(paraList[0], paraList[2]))))), fReduce(fMul(fIntToFrac_numAdd(9), fPow(paraList[0], 2)))));
...
Fraction cuDepressedPara(Fraction paraList[4], Fraction* A, Fraction* B) {
	...
	*/
	...
	c_12 = fReduce(fDiv(fReduce(fMul(fPow(paraList[1], 3), fIntToFrac_numAdd(2))), fReduce(fMul(fPow(paraList[1], 3), fIntToFrac_numAdd(27)))));
	...
	*/
	c_1 = fReduce(fSubtract(fReduce(fAdd(paraList[3], fReduce(fDiv(fReduce(fMul(fPow(paraList[1], 3), fIntToFrac_numAdd(2))), fReduce(fMul(fPow(paraList[1], 3), fIntToFrac_numAdd(27))))))), fReduce(fDiv(fReduce(fMul(paraList[1], paraList[2])), fReduce(fMul(fIntToFrac_numAdd(3), paraList[0]))))));
...
Composite_RootFraction_Fraction_Pair cuOneRealRt(Fraction delta, Fraction h_sqr, Fraction paraList[4]) {
	...
	Fraction delta_power6 , d; 
	Fraction *A, *B;

	delta_power6 = fPow(delta, 3); 
	d = cuDepressedPara(paraList, A, B );

	ans.t_cube.component1 = fReduce(fDiv(fMul_Frac_Int(d, -1 ) , fMul_Frac_Int(paraList[0], 2) ));
